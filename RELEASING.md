...
`observability/go.mod` requires `github.com/infobloxopen/hotload/v3` by
version. The `replace` directive in that file only affects building inside
this repository — **consumers resolve the `require` line**, so it must point
at a tag that exists.

For a release that touches both modules:

1. Tag the root module first:
   ```sh
   git tag v3.Y.Z && git push origin v3.Y.Z
   ```
2. Bump the require in `observability/go.mod` to the new tag:
   ```sh
