...
func StartPostgresContainer(ctx context.Context) (*PostgresContainer, error) {
	...
	if os.Getenv("DOCKER_HOST") == "" {
		...
		possibleSockets := []string{
			...
			"unix:///Users/" + os.Getenv("USER") + "/.docker/run/docker.sock", // Docker Desktop
			"unix:///Users/" + os.Getenv("USER") + "/.rd/docker.sock",         // Rancher Desktop
		}

		for _, socket := range possibleSockets {
			socketPath := socket[7:] // Remove "unix://" prefix
	...
	// Disable Ryuk (cleanup container) as it has issues with Rancher Desktop
	// We handle cleanup ourselves in TestMain
